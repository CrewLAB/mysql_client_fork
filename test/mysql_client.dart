...
void testMysqlClient(dynamic host, int port, String user, String pass, String db) {
  ...
  setUpAll(() async {
    ...
    conn = await MySQLConnection.createConnection(
      endpoint: Endpoint(host: host, port: port, username: user, password: pass, secure: true),
    );
    ...
    await conn.execute('CREATE DATABASE $db CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci');
...
''');
  ...
  test('testing bad connection', () async {
    try {
      final MySQLConnection localConn = await MySQLConnection.createConnection(
        endpoint: Endpoint(host: host, port: port, username: 'fake', password: 'fake', secure: true),
      );
  ...
  test('testing select', () async {
    ...
    // get first row
    final ResultSetRow row = await result.rowsStream.first;
    ...
    expect(row.assoc(), <String, String?>{
      'id': '1',
      'author_id': null,
      'title': 'Новая книга 😁',
      'price': '100',
    ...
    expect(row.typedAssoc(), <String, Object?>{
      'id': 1,
      'author_id': null,
      'title': 'Новая книга 😁',
      'price': 100,
  ...
  test('testing error is thrown if syntax error', () async {
    try {
      await conn.execute('SELECT * FROM book WHERES ASD id = :id', params: <String, dynamic>{'id': 1});
  ...
  test('testing delete', () async {
    final IResultSet result = await conn.execute('DELETE FROM book WHERE id = :id', params: <String, dynamic>{'id': 1});
  ...
  test('testing transaction', () async {
    await conn.transactional((MySQLConnection conn) async {
      final IResultSet result = await conn.execute(
        'INSERT INTO book (author_id, title, price, created_at, some_time) VALUES (:author, :title, :price, :created, :time)',
        params: <String, dynamic>{
          'author': null,
  ...
  test('testing double transaction', () async {
    try {
      await conn.transactional<void>((MySQLConnection conn) async {
  ...
  test('testing error is thrown if prevent double transaction', () async {
    try {
      await Future.wait(<Future<void>>[
        conn.transactional<void>((MySQLConnection conn) async {
          await conn.execute('SELECT * FROM book');
        }),
        conn.transactional<void>((MySQLConnection conn) async {
          await conn.execute('SELECT * FROM book');
  ...
  test('testing missing param', () async {
    try {
      await conn.execute('SELECT * FROM book WHERE id = :id', params: <String, dynamic>{'foo': 'bar'});
  ...
  test('testing prepared statement', () async {
    final PreparedStmt stmt = await conn.prepare('INSERT INTO book (title, price, created_at) VALUES (?, ?, ?)');

    expect(stmt.numOfParams, 3);

    IResultSet result = await stmt.execute(<dynamic>['Some title 1', 200, '2022-04-02 00:00:00']);

    expect(result.affectedRows.toInt(), 1);
    expect(result.lastInsertID.toInt(), 3);

    result = await stmt.execute(<dynamic>['Some title 2', 200, '2022-04-02 00:00:00']);
  ...
  test('testing string encoding in prepared statements', () async {
    final PreparedStmt stmt = await conn.prepare(
      'INSERT INTO book (author_id, title, price, created_at) VALUES (?, ?, ?, ?)',
    );

    final IResultSet result = await stmt.execute(<dynamic>[null, '中文标题', 120, '2022-01-01']);
    await stmt.deallocate();
  ...
  test('testing prepared stmt select', () async {
    final PreparedStmt stmt = await conn.prepare('SELECT * FROM book WHERE title = ?');

    final IResultSet result = await stmt.execute(<dynamic>['Some title 2']);
  ...
  test('testing empty result set', () async {
    final IResultSet result = await conn.execute('SELECT * FROM book WHERE id = 99999');
    expect(result.numOfRows, 0);
  });

  test('testing empty result for prepared statement', () async {
    final PreparedStmt stmt = await conn.prepare('SELECT * FROM book WHERE id = 99999');
  ...
  test('testing multiple statements', () async {
    ...
    final List<IResultSet> resultSetsList = resultSets.toList();
  ...
  test('testing column types mapping', () async {
    const String tableName = 'column_types_test_123';
    ...
    await conn.execute("""
        ...
        """);
    final IResultSet response = await conn.execute('SELECT * FROM $tableName');
    for (final ResultSetRow row in response.rows) {
      final Map<String, dynamic> typedAssoc = row.typedAssoc();
    ...
    final IResultSet groupedResponse = await conn.execute('''
      SELECT 
      col_pk, 
      ...
      ''');
    for (final ResultSetRow row in groupedResponse.rows) {
      final Map<String, dynamic> typedAssoc = row.typedAssoc();
      expect(typedAssoc['col_pk'].runtimeType, int);
  ...
  test(
    ...
    () async {
      ...
      log('Inserting $stressTestRows rows...');

      for (int x = 0; x < stressTestRows; x++) {
        await stmt.execute(<dynamic>['Some title $x', x, '2022-04-02 00:00:00']);
      }
      ...
      await for (final ResultSetRow _ in result.rowsStream) {
        receivedRows++;
      }
